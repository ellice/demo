...
func (test *TestMysql)Create(){
    ...
    stmt,err := test.db.Prepare("insert into user(name,age)values(?,?)");
    if err!=nil {
...
func (test *TestMysql)Update(){
	...
	stmt,err := test.db.Prepare("update user set name=?,age=? where age=?");
	if err!=nil {
		fmt.Println(err.Error());
		return;
	}
...
func (test *TestMysql)Read(){
	...
	rows,err := test.db.Query("select id,name,age from user limit 0,5");
	if err!=nil {
		fmt.Println(err.Error());
		return;
	}
...
func (test *TestMysql)Delete(){
	...
	stmt,err := test.db.Prepare("delete from user where age=?");
	if err!=nil {
		fmt.Println(err.Error());
		return;
	}
